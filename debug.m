...
% Caricamento dati dal file WORKSPACE.mat
load('WORKSPACE.mat');

% Generazione del vettore con 46 valori casuali tra 5 e 20
numStazioni_debug_AD = randi([5, 20], 46, 1); % Vettore colonna 46x1
% Creazione della matrice 46x2
numStazioni_debug_AD_matrix = [numStazioni_debug_AD - 1, numStazioni_debug_AD + 1];
% Visualizzazione dei risultati
disp('Vettore AD originale:');
disp(numStazioni_debug_AD);
disp('Matrice risultante:');
disp(numStazioni_debug_AD_matrix);

numStazioni_debug_AP = randi([5, 20], 46, 1); % Vettore colonna 46x1
% Creazione della matrice 46x2
numStazioni_debug_AP_matrix = [numStazioni_debug_AP - 1, numStazioni_debug_AP + 1];
% Visualizzazione dei risultati
disp('Vettore AP originale:');
disp(numStazioni_debug_AP);
disp('Matrice risultante:');
disp(numStazioni_debug_AP_matrix);

for noiseIdx = 1:length(debugNoise)
    noiseI = debugNoise(noiseIdx);

    % Generazione stazioni e calcolo solo per RN_STAZIONI = [19 20] e stdDevCoeff = 0.8
    fprintf('Elaborazione: Noise = %.1f, StdDev = %.1f\n', noiseI, debugmeanstdDev);
...
% \\\\\\\\\\ SEZIONE CALCOLI //////////
results_AD_debug = cell(length(debugNoise), 1);
results_AP_debug = cell(length(debugNoise), 1);
