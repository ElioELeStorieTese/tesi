...
% checkpoint ciclo
totalIterations = length(noiseRange) * length(stdDevCoeff) * length(numStazRange); % Numero totale di iterazioni
...
for noiseIdx = 1:length(noiseRange)
    ...
    for stdDevIdx = 1:length(stdDevCoeff)
        ...
        for stationIdx = 1:length(numStazRange)
            fprintf('Elaborazione: Noise = %.1f, StdDev = %.1f\n', noiseI, stdDevJ);
            stazLat_AD = [];
            stazLon_AD = [];
            cityIndex_AD = [];
            stazLat_AP = [];
            ...
            file_path_vel_AD = sprintf('SV_AD_Noise%.1f_StDev%.1f_#Staz%d.txt', noiseI, stdDevJ, length(stazLat_AD));
            file_path_vel_AP = sprintf('SV_AP_Noise%.1f_StDev%.1f_#Staz%d.txt', noiseI, stdDevJ, length(stazLat_AP));
            ...
            % Checkpoint
            currentIteration = (noiseIdx - 1) * length(stdDevCoeff) * length(numStazRange) + (stdDevIdx - 1) * length(numStazRange) + stationIdx;
...
% Matrici per grafico
overlap_AD_4graph = zeros(length(noiseRange), length(stdDevCoeff), length(numStazRange));
overlap_AP_4graph = zeros(length(noiseRange), length(stdDevCoeff), length(numStazRange));
N_samples_overlap = 1e5;
LoLa_RES = 0.5;         % Risoluzione coordinate polari
N_NORM_BINS = 20;       % Numero di bin per la normalizzazione
...
for noiseIdx = 1:length(noiseRange)
    for stdDevIdx = 1:length(stdDevCoeff)
        for stationIdx = 1:length(numStazRange), 
            overlapResults_AD{noiseIdx, stdDevIdx, stationIdx} = FNC_OVL_2EVs(EV_AD, results_AD{noiseIdx, stdDevIdx, stationIdx}.EV_AD,N_samples_overlap,LoLa_RES = 0.5,N_NORM_BINS);
            overlapResults_AP{noiseIdx, stdDevIdx, stationIdx} = FNC_OVL_2EVs(EV_AP, results_AP{noiseIdx, stdDevIdx, stationIdx}.EV_AP,N_samples_overlap,LoLa_RES = 0.5,N_NORM_BINS);
